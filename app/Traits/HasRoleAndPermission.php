...

{
     ...
     * @param array|App\Models\Role[]|\Illuminate\Database\Eloquent\Collection $roles
        ...
        if (config('roles.inheritance')) {
            return $permissionModel
                ...
                ->groupBy([
                    ...
                    'permissions.status',  // Add the 'status' column here
                    'pivot_created_at',
                    'pivot_updated_at'
                ]);
        } else {
    ...
    public function getPermissions()
    {
        if (!$this->permissions) {
            ...
            $this->permissions = $rolePermissions->merge($userPermissions);
        }
