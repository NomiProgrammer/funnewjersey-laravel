...
@section('admin-content')
    @php
        $pageName = 'Edit Deals';
        $pageName2 = 'Edit New Deals Records';
    @endphp
            ...
            <div class="container-fluid">
                <div class="row">
                    <div class="col-12">
                       ...
                       <form action="{{ route('deals.update', $deals->id) }}" method="POST" enctype="multipart/form-data">
    @csrf
    @method('PUT')
    ...
    <div class="card-body">
        <div class="row">
            ...
            <div class="form-group col-md-6">
                ...
                <input type="text" class="form-control" required name="title" value="{{ old('title', $deals->title) }}">
            </div>

            {{-- Deal % Off or Discount --}}
            <div class="form-group col-md-6">
                <label for="price">Deal % Off or Discount</label>
                <input type="text" class="form-control" required name="price" value="{{ old('price', $deals->price) }}">
            </div>

            {{-- Deal Limit --}}
            <div class="form-group col-md-6">
                <label for="deal_limit">Deal Limit</label>
                <input type="number" class="form-control" required name="deal_limit" value="{{ old('deal_limit', $deals->deal_limit) }}">
            </div>

            {{-- Terms & Conditions --}}
            <div class="form-group col-md-12">
                <label for="terms">Terms & Conditions</label>
                <textarea required name="terms" id="content" class="form-control" rows="4">{{ old('terms', $deals->terms) }}</textarea>
            </div>

            {{-- Content --}}
            <div class="form-group col-md-12">
                <label for="description">Content</label>
                <textarea required name="description" id="content" class="form-control" rows="5">{{ old('description', $deals->description) }}</textarea>
            </div>
            ...
            <div class="form-group col-md-6">
                ...
                @if($deals->featured_img)
                    <p class="mt-2"><strong>Current Image:</strong></p>
                    <img src="{{ asset('front_assets/uploads/deals/' . $deals->featured_img) }}" alt="Featured Image" width="150">
                @endif
            </div>

            {{-- Hidden Type --}}
            <input type="hidden" required name="type" value="deal">
        </div>
    </div>
