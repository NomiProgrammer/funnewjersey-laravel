...
@section('page_title', 'Manage Blog Articles')
...
@section('admin-content')
    @php
        $pageName = 'Manage Blog Articles';
        $pageName2 = 'Blog Articles Records';
    @endphp
...
<script>
    ...
    <script>
        $(document).on('click', '.delete-blogdelete', function(e) {
            e.preventDefault();

            let id = $(this).data('id');
            if (confirm('Are you sure you want to delete this Blog?')) {
                $.ajax({
                    url: "{{ url(app()->getLocale() . '/admin/blog/destroy') }}/" + id,
                    type: 'DELETE',
                    data: {
                        _token: '{{ csrf_token() }}'
                    },
